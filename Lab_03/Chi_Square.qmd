...
1. Whether there are the same number of each flavor of gummy bear in a package
    ...
    - Different categories = different flavors of gummy bears
...
* In the `chisq.test` function, the default null hypothesis is the equal category hypothesis that predicts the same number of observations for each category.
...
The **risk** of a delay with DL is 1.16x higher with DL than with AA
...
When writing reports with R Markdown or Quarto, we might want to use mathematical notation or symbols.
...
* See <https://latexmathsymbols.com/> for a list of the most useful characters
